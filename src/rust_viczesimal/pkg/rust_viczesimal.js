...
export function dec_to_vicz(decimal) {
    try {
        ...
        wasm.dec_to_vicz(retptr, decimal);
...
* @returns {string}
*/
export function calculate_vicz_expression(expression) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passStringToWasm0(expression, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        wasm.calculate_vicz_expression(retptr, ptr0, len0);
        var r0 = getInt32Memory0()[retptr / 4 + 0];
        var r1 = getInt32Memory0()[retptr / 4 + 1];
        return getStringFromWasm0(r0, r1);
    } finally {
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL('rust_viczesimal_bg.wasm', import.meta.url);
    }
    const imports = getImports();
